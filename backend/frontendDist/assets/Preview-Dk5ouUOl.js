...
import {
  ...
  s as oe,
  u as se,
} from "./index-qq9Unyga.js";
import { t as ce } from "./userAuthStore-BdWQrsQ-.js";
import { t as le } from "./workPackageStore-DTiMqbPM.js";
var ue = x(`circle-minus`, [
  ...
  S = b(`create`, {
    ...
    actions: {
      ...
      addCount(e) {
        ...
        let i = r.innerItems[e.id];
        return (i && (i.count += e.pulse), t.items);
      },
      pushpreviewItem(e) {
        if (
          (console.log(`token`, e),
          !this.previewCase || !this.listItem || this.addItemCounter == null)
        )
          return;
        let t = this.previewCase[e.caseId].pockets[e.pocketId];
        this.addItemCounter++;
        ...
        if (e.parentId == null) t.items[n.id] = n;
        else {
          let r = t.items[e.parentId]?.innerItems;
          if (r) return ((r[n.id] = n), { item: r[n.id], parent: e.parentId });
        }
        return (console.log(`pocket.items`, t), { item: t.items[n.id] });
      },
      addBookmark(e) {
      ...
      deletepreviewItem(e) {
        ...
        if (e.parentId == null) return (delete t.items[n], t.items);
        if (e.parentId != null) {
          let n = t.items[e.parentId];
          return (delete n?.innerItems?.[e.id], t.items);
        }
      },
      addPreviewCase(e) {
        if (e.reverse) {
...
var T = { type: `error`, data: `parser error` },
  pe =
...
for (let e = 0; e < 64; e++) D[be.charCodeAt(e)] = e;
var xe = (e) => {
    let t = e.length * 0.75,
      n = e.length,
      r,
    ...
    for (r = 0; r < n; r += 4)
      ((a = D[e.charCodeAt(r)]),
        (o = D[e.charCodeAt(r + 1)]),
        (s = D[e.charCodeAt(r + 2)]),
        (c = D[e.charCodeAt(r + 3)]),
        (u[i++] = (a << 2) | (o >> 4)),
        (u[i++] = ((o & 15) << 4) | (s >> 2)),
        (u[i++] = ((s & 3) << 6) | (c & 63)));
    return l;
    ...
    transform(o, s) {
      for (n.push(o); ; ) {
        if (r === 0) {
          ...
          if (k(n) < i) break;
          let e = A(n, i);
          (s.enqueue(O(a ? e : ke.decode(e), t)), (r = 0));
        }
        if (i === 0 || i > e) {
          s.enqueue(T);
          break;
...
function P(e, t) {
  t.useNativeTimers
    ? ((e.setTimeoutFn = Pe.bind(N)), (e.clearTimeoutFn = Fe.bind(N)))
...
var He = class extends Error {
  ...
  F = class extends j {
    ...
    onData(e) {
      let t = O(e, this.socket.binaryType);
      this.onPacket(t);
    }
    onPacket(e) {
...
if (((I.requestsCount = 0), (I.requests = {}), typeof document < `u`)) {
  if (typeof attachEvent == `function`) attachEvent(`onunload`, Je);
  else if (typeof addEventListener == `function`) {
    let e = `onpagehide` in N ? `pagehide` : `unload`;
    addEventListener(e, Je, !1);
  }
}
function Je() {
  for (let e in I.requests)
...
function Ze(e) {
  ...
  if (!t)
    try {
      return new N[[`Active`, `Object`].join(`X`)](`Microsoft.XMLHTTP`);
    } catch {}
}
var Qe =
    typeof navigator < `u` &&
  ...
  et = {
    websocket: class extends $e {
      createSocket(e, t, n) {
        return Qe ? new L(e, t, n) : t ? new L(e, t) : new L(e);
      }
      doWrite(e, t) {
        this.ws.send(t);
      }
    },
    webtransport: class extends F {
      get name() {
        return `webtransport`;
      }
      doOpen() {
...
var B = class e extends j {
  constructor(e, t) {
    ...
    } else t.host && (t.hostname = R(t.host).host);
    (P(this, t),
      (this.secure =
        t.secure == null
...
var gt = se((e, t) => {
  ...
  G = (0, re(vt()).default)(`socket.io-parser`),
  bt = [
    `connect`,
    `connect_error`,
    `disconnect`,
...
function Z(e, t) {
  ...
  let n = ct(e, t.path || `/socket.io`),
    ...
    c;
  return (
    s ? (c = new jt(r, t)) : (X[i] || (X[i] = new jt(r, t)), (c = X[i])),
    n.query && !t.query && (t.query = n.queryKey),
    c.socket(n.path, t)
  );
}
Object.assign(Z, { Manager: jt, Socket: At, io: Z, connect: Z });
var Mt = `http://localhost:3000`,
  Nt = S(),
  Pt = Z(Mt),
  Ft = () => ({
    sendAlteration: (e) => {
      Pt.emit(`work:alteration`, e);
    },
    sendDb: (e) => {
      !Nt.workIdGetter || e.length;
      let t = JSON.stringify({ workId: Nt.workIdGetter, data: e });
      return new Promise((e) => {
        Pt.emit(`work:save`, t, (t) => {
          e(t);
        });
      });
    },
  ...
  Rt = S(),
  zt = () => {
    if (Rt.workIdGetter == null) return;
    let e = [],
      t = (t) => {
        let n = {
          id: `id_${e.length}`,
        ...
        (e.length == 0 && i(), e.push(n));
      },
      n = async () => {
        (await Lt.sendDb(e)).success == 1
          ? (e.length = 0)
          : It.showAlert(`保存に失敗しました`, !0);
      },
      r = () => {
        e.length = 0;
      },
    ...
    return { push: t, pop: n, deleteQueue: r };
  },
  Q = () => {
    let e = zt(),
      t = S();
    return {
      hydrateCreateState: (e) => {
        ...
        let n = t.staticItemData,
          ...
          s = Object.fromEntries(
            Object.entries(o).map(([e, t]) => [
              e,
              { ...t, bookmark: a.bookmarks.includes(t.itemId) },
            ]),
          ),
          c = (e) =>
            Object.fromEntries(
              Object.values(e).map((e) => {
                let { bookmark: t, ...n } = s[e.itemId],
                  r = s[e.itemId].isStorage
                    ? {
                        ...n,
                        innerItems: e.innerItems ? c(e.innerItems) : {},
                        count: e.count,
                        id: e.id,
                      }
        ...
        return (
          console.log(`previewDatas.mainLuggage`, i.mainLuggage),
          {
        ...
        switch (n.alterationType) {
          case `previewItems_additem`: {
            ...
            ((r.path = [
              ...
              e.parent != null && r.path.push(e.parent),
              (r.value = e.item),
              (r.type = `objectPush`));
            break;
          }
          ...
          case `previewCases_addCase`: {
            ...
            if (!e) return;
            let i = { id: e.caseId, caseType: e.caseType };
  ...
  Ht = () => ({
    getWork: async (e) => {
      ...
      let n = { ...t.data, workName: t.name, workId: t.id };
      return (console.log(`parse`, n), n);
    },
    ...
    let e = S(),
      ...
      a = async () => {
        ...
        return (
          ...
          Ut.selectedPackageIdStore(a.workId),
          `none`
        );
      },
      o = async () => {
        let t = Ut.selectedPackageIdGetter;
        ...
        if ((console.log(`data`, n), n == null)) throw Error();
        await Q().initCreateStaticData();
        let c = i.hydrateCreateState(n);
        return (
          (a = c.vuepreviewData),
          (o = c.vueItemList),
          (s = c.addItemCounter),
  ...
  Wt = v({
    ...
    setup(e, { expose: t }) {
      ...
      let n = e,
        ...
        a = r.iconMap;
      function o(e) {
        let t = { id: n.item.id, popCaseId: n.caseId, popPocketId: n.pocketId };
    ...
    u(
      ...
      [
        l(`div`, Qt, [
          ...
          l(`div`, tn, [
            Object.keys(r.pocket.items).length === 0
            ...
            u(
              ...
              o(
                ...
                (t) => (
                  s(),
                  u(`div`, { key: t.id, class: `item-card` }, [
                    e(
                      i.PreviewItem,
                      {
...
export { S as a, Bt as i, on as n, $ as r, Vn as t };
//# sourceMappingURL=Preview-Dk5ouUOl.js.map
