...
import { t as g } from "./BaseButton-DgJxAwXf.js";
import { n as _, o as v } from "./index-DPNatowj.js";
import { t as y } from "./userAuthstore-BS2VQYb3.js";
var b = y(),
  x = `http://localhost:3000/auth/2fa`,
  S = () => ({
    setup: async () => {
      let e = await (
        await fetch(`${x}/setup`, {
          ...
          body: JSON.stringify({ userId: b.userId, email: b.userEmail }),
        })
      ).json();
      return (console.log(`data:` + JSON.stringify(e)), e);
    },
    verification2fa: async (e) => {
      let t = await (
        await fetch(`${x}/verification`, {
          method: `POST`,
          headers: { "Content-Type": `application/json` },
          body: JSON.stringify({ userId: b.userId, code: e }),
        })
      ).json();
      return (t && b.set2fa(), t);
    },
  ...
  e.QrCode = (function () {
    function a(e, t, n, i) {
      if (
        ...
        (this.modules = []),
        (this.isFunction = []),
        e < a.MIN_VERSION || e > a.MAX_VERSION)
      )
      ...
      if ((this.drawCodewords(c), i == -1))
        for (var l = 1e9, s = 0; s < 8; s++) {
    ...
    return (
      ...
      (a.encodeSegments = function (e, n, o, s, c, l) {
        if (
          ...
          c === void 0 && (c = -1),
          l === void 0 && (l = !0),
          !(a.MIN_VERSION <= o && o <= s && s <= a.MAX_VERSION) ||
            c < -1 ||
            c > 7)
        )
        ...
        var u, d;
        for (u = o; ; u++) {
          var f = a.getNumDataCodewords(u, n) * 8,
            p = i.getTotalBits(e, u);
          if (p <= f) {
            d = p;
            break;
          }
        ...
        for (
          var m = 0, h = [a.Ecc.MEDIUM, a.Ecc.QUARTILE, a.Ecc.HIGH];
          m < h.length;
          m++
        ) {
          var g = h[m];
        ...
        for (var _ = [], v = 0, y = e; v < y.length; v++) {
          var b = y[v];
          (t(b.mode.modeBits, 4, _),
            t(b.numChars, b.mode.numCharCountBits(u), _));
        ...
        (r(_.length <= w),
          t(0, Math.min(4, w - _.length), _),
          t(0, (8 - (_.length % 8)) % 8, _),
          r(_.length % 8 == 0));
      ...
      (a.prototype.drawFunctionPatterns = function () {
        ...
        for (
          var t = this.getAlignmentPatternPositions(), n = t.length, e = 0;
          e < n;
          e++
        )
          for (var r = 0; r < n; r++)
      ...
      (a.prototype.drawFormatBits = function (e) {
        for (
          var t = (this.errorCorrectionLevel.formatBits << 3) | e, i = t, a = 0;
          a < 10;
          a++
        )
          i = (i << 1) ^ ((i >>> 9) * 1335);
        ...
        for (var a = 0; a < 8; a++)
          this.setFunctionModule(this.size - 1 - a, 8, n(o, a));
        for (var a = 8; a < 15; a++)
          this.setFunctionModule(8, this.size - 15 + a, n(o, a));
        ...
        for (var n = -2; n <= 2; n++)
          for (var r = -2; r <= 2; r++)
            this.setFunctionModule(
              e + r,
              t + n,
              Math.max(Math.abs(r), Math.abs(n)) != 1,
            );
      }),
      ...
      (a.prototype.addEccAndInterleave = function (e) {
        ...
        if (e.length != a.getNumDataCodewords(t, n))
          throw RangeError(`Invalid argument`);
        for (
          var i = a.NUM_ERROR_CORRECTION_BLOCKS[n.ordinal][t],
            o = a.ECC_CODEWORDS_PER_BLOCK[n.ordinal][t],
            s = Math.floor(a.getNumRawDataModules(t) / 8),
            c = i - (s % i),
            l = Math.floor(s / i),
            u = [],
            d = a.reedSolomonComputeDivisor(o),
            f = 0,
            p = 0;
          f < i;
          f++
      ...
      (a.prototype.drawCodewords = function (e) {
        ...
        for (var t = 0, i = this.size - 1; i >= 1; i -= 2) {
          ...
          for (var o = 0; o < this.size; o++)
            for (var s = 0; s < 2; s++) {
              var c = i - s,
                l = (i + 1) & 2 ? o : this.size - 1 - o;
              !this.isFunction[l][c] &&
                t < e.length * 8 &&
                ((this.modules[l][c] = n(e[t >>> 3], 7 - (t & 7))), t++);
            }
        }
        r(t == e.length * 8);
      }),
      ...
      (a.prototype.getPenaltyScore = function () {
        for (var e = 0, t = 0; t < this.size; t++) {
          for (
            var n = !1, i = 0, o = [0, 0, 0, 0, 0, 0, 0], s = 0;
            s < this.size;
            s++
          )
            this.modules[t][s] == n
              ? (i++, i == 5 ? (e += a.PENALTY_N1) : i > 5 && e++)
              : (this.finderPenaltyAddHistory(i, o),
                n || (e += this.finderPenaltyCountPatterns(o) * a.PENALTY_N3),
                (n = this.modules[t][s]),
                (i = 1));
        ...
        for (var s = 0; s < this.size; s++) {
          for (
            var n = !1, c = 0, o = [0, 0, 0, 0, 0, 0, 0], t = 0;
            t < this.size;
            t++
          )
            this.modules[t][s] == n
              ? (c++, c == 5 ? (e += a.PENALTY_N1) : c > 5 && e++)
              : (this.finderPenaltyAddHistory(c, o),
                n || (e += this.finderPenaltyCountPatterns(o) * a.PENALTY_N3),
                (n = this.modules[t][s]),
                (c = 1));
        ...
        for (var t = 0; t < this.size - 1; t++)
          for (var s = 0; s < this.size - 1; s++) {
            ...
            l == this.modules[t][s + 1] &&
              l == this.modules[t + 1][s] &&
              l == this.modules[t + 1][s + 1] &&
              (e += a.PENALTY_N2);
          }
        for (var u = 0, d = 0, f = this.modules; d < f.length; d++)
          u = f[d].reduce(function (e, t) {
            return e + +!!t;
          }, u);
        var p = this.size * this.size,
          m = Math.ceil(Math.abs(u * 20 - p * 10) / p) - 1;
        return (
          r(0 <= m && m <= 9),
          (e += m * a.PENALTY_N4),
          r(0 <= e && e <= 2568888),
      ...
      (a.getNumDataCodewords = function (e, t) {
        return (
          ...
          a.ECC_CODEWORDS_PER_BLOCK[t.ordinal][e] *
            a.NUM_ERROR_CORRECTION_BLOCKS[t.ordinal][e]
        );
      }),
      (a.reedSolomonComputeDivisor = function (e) {
        ...
        for (var r = 1, n = 0; n < e; n++) {
          for (var i = 0; i < t.length; i++)
            ((t[i] = a.reedSolomonMultiply(t[i], r)),
              i + 1 < t.length && (t[i] ^= t[i + 1]));
      ...
      (a.reedSolomonMultiply = function (e, t) {
        ...
        for (var n = 0, i = 7; i >= 0; i--)
          ((n = (n << 1) ^ ((n >>> 7) * 285)), (n ^= ((t >>> i) & 1) * e));
        return (r(n >>> 8 == 0), n);
      }),
  ...
  var i = (function () {
    ...
    return (
      ...
      (e.makeAlphanumeric = function (n) {
        ...
        return (
          i < n.length && t(e.ALPHANUMERIC_CHARSET.indexOf(n.charAt(i)), 6, r),
          new e(e.Mode.ALPHANUMERIC, n.length, r)
        );
      }),
...
function I(e, t, n) {
  var r = t > 0 ? e[t - 1][n] : !1,
    i = t < e.length - 1 ? e[t + 1][n] : !1,
    a = n > 0 ? e[t][n - 1] : !1,
    o = n < e[t].length - 1 ? e[t][n + 1] : !1;
  return { nw: !r && !a, ne: !r && !o, se: !i && !o, sw: !i && !a };
}
function L(e, t, n) {
  (t === void 0 && (t = 0), n === void 0 && (n = 0));
  for (var r = [], i = Math.min(n, 0.5), a = 0; a < e.length; a++)
    for (var o = 0; o < e[a].length; o++)
      if (e[a][o]) {
        var s = I(e, a, o),
          ...
          f = o + t,
          p = a + t;
        (r.push(`M${f + (c ? i : 0)} ${p}`, `L${f + 1 - (l ? i : 0)} ${p}`),
          l && r.push(`A${i} ${i} 0 0 1 ${f + 1} ${p + i}`),
          r.push(`L${f + 1} ${p + 1 - (u ? i : 0)}`),
          u && r.push(`A${i} ${i} 0 0 1 ${f + 1 - i} ${p + 1}`),
          r.push(`L${f + (d ? i : 0)} ${p + 1}`),
          d && r.push(`A${i} ${i} 0 0 1 ${f} ${p + 1 - i}`),
          r.push(`L${f} ${p + (c ? i : 0)}`),
          c && r.push(`A${i} ${i} 0 0 1 ${f + i} ${p}`),
          r.push(`z`));
      }
  return r.join(``);
}
function R(e, t) {
  ...
  for (var n = [], r = 0; r < e.length; r++)
    for (var i = e[r], a = null, o = 0; o < i.length; o++) {
      var s = i[o];
      if (!s && a !== null) {
      ...
      if (o === i.length - 1) {
        ...
        a === null
          ...
          : n.push(`M${a + t},${r + t} h${o + 1 - a}v1H${a + t}z`);
        continue;
      }
      s && a === null && (a = o);
    }
...
function B(e) {
  var t = d(function () {
    ...
    n = d(function () {
      var t = F(e.level) ? e.level : O;
      return T.QrCode.encodeText(e.value, N[t]).getModules();
    }),
    r = d(function () {
      return n.value.length + t.value * 2;
    }),
    i = d(function () {
      return e.radius > 0 ? L(n.value, t.value, e.radius) : R(n.value, t.value);
    }),
    a = d(function () {
      if (!e.imageSettings.src) return null;
      var r = z(n.value, e.size, t.value, e.imageSettings);
      return {
        x: r.x + t.value,
        y: r.y + t.value,
        width: r.w,
  ...
  return {
    ...
    fgPath: i,
    imageProps: a,
    imageBorderProps: d(function () {
      if (!e.imageSettings.excavate || !a.value) return null;
      var t = M / (e.size / r.value);
      return {
        x: a.value.x - t,
        y: a.value.y - t,
        width: a.value.width + t * 2,
        height: a.value.height + t * 2,
        borderRadius: a.value.borderRadius,
      };
    }),
  };
}
var V = {
    ...
    margin: { type: Number, required: !1, default: A },
    imageSettings: {
      type: Object,
      required: !1,
      default: function () {
  ...
  U = t({
    ...
    setup: function (e) {
      var t = B(e),
        ...
        i = t.fgPath,
        a = t.imageProps,
        o = t.imageBorderProps,
        s = D(e.id),
        c = `qrcode.vue-gradient-${s}`,
        l = `qrcode.vue-logo-clip-path-${s}`,
        u = d(function () {
      ...
      return function () {
        return n(
          ...
          [
            n(`defs`, {}, [u.value, f.value]),
            n(`rect`, { width: `100%`, height: `100%`, fill: e.background }),
            n(`path`, {
              fill: e.gradient ? `url(#${c})` : e.foreground,
              d: i.value,
            }),
            o.value &&
              n(`rect`, {
                ...
                fill: e.background,
                rx: o.value.borderRadius,
                ry: o.value.borderRadius,
              }),
            e.imageSettings.src &&
              a.value &&
              n(
                `image`,
                C(
                  C({ href: e.imageSettings.src }, a.value),
                  a.value.borderRadius > 0 ? { "clip-path": `url(#${l})` } : {},
                ),
              ),
          ],
        );
      ...
      var i = B(e),
        ...
        _ = function () {
          var t = e.size,
            ...
            i = e.gradient,
            s = e.gradientType,
            l = e.gradientStartColor,
            f = e.gradientEndColor,
            _ = m.value;
          if (_) {
            var v = _.getContext(`2d`);
            if (v) {
              var y = h.value,
                b = (typeof window < `u` && window.devicePixelRatio) || 1,
                x = (t / c.value) * b;
              if (
                ((_.height = _.width = t * b),
                v.setTransform(x, 0, 0, x, 0, 0),
                (v.fillStyle = n),
                v.fillRect(0, 0, c.value, c.value),
                i)
              ) {
                var S = void 0;
                ((S =
                  s === `linear`
                    ? v.createLinearGradient(0, 0, c.value, c.value)
                    : v.createRadialGradient(
                        c.value / 2,
                        c.value / 2,
                        0,
                        c.value / 2,
              ...
              if (
                (P
                  ? v.fill(new Path2D(u.value))
                  : o.value.forEach(function (e, t) {
                ...
                if (p.value) {
                  var C = p.value;
                  ((v.fillStyle = e.background),
                    g(v, C.x, C.y, C.width, C.height, C.borderRadius),
                    v.fill());
                }
                var w = d.value.borderRadius;
                w > 0
                  ? (v.save(),
                    g(
                      v,
  ...
  K = t({
    __name: `SplitQr2fa`,
    props: { otpauthUrl: { type: null, required: !0 } },
    setup(e, { expose: t }) {
      t();
      let n = e;
      r(() => {
        console.log(`grCode:` + n.otpauthUrl);
      });
      let i = { props: n, QrcodeVue: G };
      return (
  ...
  q = { class: `left` };
function J(t, n, r, a, o, s) {
  return (
    e(),
    h(`div`, q, [
      i(a.QrcodeVue, { value: r.otpauthUrl, size: 350, level: `H` }, null, 8, [
        `value`,
      ]),
    ])
  );
...
var Y = p(K, [
    [`render`, J],
    [`__scopeId`, `data-v-6f18ba04`],
    [
      `__file`,
      `/Users/azumakoutaira/workspace/moti_nabi/frontend/src/features/auth/components/2fa/left/SplitQr2fa.vue`,
    ],
  ]),
  X = 6,
  Z = t({
    __name: `AuthProcedureScreen`,
    emits: [`completion:modelValue`],
    setup(e, { expose: t, emit: n }) {
      ...
      let i = n,
        ...
        o = f(),
        s = () => {
          a.value.length == X && i(`completion:modelValue`, a.value);
        };
      r(() => {
        let e = navigator.userAgent.toLowerCase();
        e.indexOf(`iphone`) > 0 || e.indexOf(`ipad`) > 0
          ? (o.value = `https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=ja`)
          : e.indexOf(`android`) > 0
            ? (o.value = `https://apps.apple.com/jp/app/google-authenticator/id388497605`)
            : (o.value = `https://chromewebstore.google.com/detail/authenticator/bhghoamapcdpbohphigoooaddinpkbai?hl=ja`);
      });
      let c = {
        emit: i,
        otp: a,
  ...
  ee = [`href`],
  te = { class: `bottom` };
function ne(t, n, r, s, l, d) {
  let f = a(`v-otp-input`);
  return (
    e(),
    h(`div`, Q, [
      u(`div`, $, [
        (n[2] ||= u(`h1`, null, `2段階認証の手順`, -1)),
        u(`li`, null, [
          u(`p`, null, [
            u(
              `a`,
            ...
            (n[1] ||= m(`を使ってQRコードを読み取る `, -1)),
          ]),
        ]),
        (n[3] ||= c(
          `<li data-v-13ee7bb5><p data-v-13ee7bb5>表示された6桁のコードを入力する</p></li><h3 style="margin-top:30px;" data-v-13ee7bb5>Authenticator DL用QRコード</h3><section class="qrcode-section" data-v-13ee7bb5><div class="qrcode" style="margin-right:auto;" data-v-13ee7bb5><img src="/images/qr/authenticator-google.png" class="qr-image" data-v-13ee7bb5><div data-v-13ee7bb5>Andoroid用</div></div><div class="qrcode" style="margin-left:auto;" data-v-13ee7bb5><img src="/images/qr/authenticator-apple.png" class="qr-image" data-v-13ee7bb5> IOS/Apple用</div></section>`,
          3,
        )),
      ]),
      u(`div`, te, [
        i(
          ...
          {
            modelValue: s.otp,
            "onUpdate:modelValue": (n[0] ||= (e) => (s.otp = e)),
            length: s.otpLength,
            variant: `outlined`,
            type: `number`,
            class: `custom-otp`,
        ...
        i(
          s.BaseButton,
          { class: `button`, onClick: s.onOtp },
          { default: o(() => [...(n[4] ||= [m(`確認する`, -1)])]), _: 1 },
        ),
      ]),
    ])
  );
...
var re = p(Z, [
    [`render`, ne],
    [`__scopeId`, `data-v-13ee7bb5`],
    [
      `__file`,
      `/Users/azumakoutaira/workspace/moti_nabi/frontend/src/features/auth/components/2fa/right/AuthProcedureScreen.vue`,
    ],
  ]),
  ie = t({
    __name: `2faPage`,
    setup(e, { expose: t }) {
      ...
      let n = _(),
        ...
        o = f(``),
        s = S(),
        c = n.options.history.state.back,
        l = async (e) => {
          (await s.verification2fa(e))
            ? (i.showAlert(`2段階認証に成功しました`),
              c ?? n.push(`/`),
              n.push(`${c}`))
            : i.showAlert(`2段階認証に失敗しました`, !0);
        };
      r(async () => {
        !u.isAuthenticated ||
          u.isTempAuthenticated ||
          (a.value = await s.setup());
      });
      let u = y(),
        d = {
          router: n,
  ...
  ae = { class: `page` };
function oe(t, n, r, a, o, s) {
  return (
    e(),
    h(`div`, ae, [
      i(a.SplitQr2fa, { otpauthUrl: a.use2faResponse?.otpauthUrl }, null, 8, [
        `otpauthUrl`,
      ]),
      i(
        a.AuthProcedureScreen,
        {
          modelValue: a.otp,
          "onUpdate:modelValue": (n[0] ||= (e) => (a.otp = e)),
          "onCompletion:modelValue": a.sendOtp,
        },
        null,
        8,
        [`modelValue`],
...
var se = p(ie, [
  [`render`, oe],
  [`__scopeId`, `data-v-8e447096`],
  [
    `__file`,
    `/Users/azumakoutaira/workspace/moti_nabi/frontend/src/page/auth/2faPage.vue`,
  ],
]);
